...
# macOS can inject proxy settings into every Java process via system properties.
# Canal Adapter uses JDBC sockets for local targets; a global SOCKS proxy can make
# localhost PostgreSQL connections time out, so keep adapter runtime local-safe.
...
if [ ! -d $base/logs/adapter ] ; then
	mkdir -p $base/logs/adapter
fi
...
if [ $JAVA_VERSION -ge 11 ] ; then
  JAVA_OPTS="$JAVA_OPTS -Xss1m -XX:-OmitStackTraceInFastThrow"
else
...
JAVA_OPTS="$JAVA_OPTS -Dhttp.proxyHost= -Dhttps.proxyHost= -DsocksProxyHost="
JAVA_OPTS="$JAVA_OPTS -Dhttp.proxyPort=0 -Dhttps.proxyPort=0 -DsocksProxyPort=0"
JAVA_OPTS="$JAVA_OPTS -Dhttp.nonProxyHosts=localhost|127.0.0.1|::1 -DsocksNonProxyHosts=localhost|127.0.0.1|::1"
